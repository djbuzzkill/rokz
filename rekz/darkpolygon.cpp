...
struct PolygonDraw : public DrawSequence {
  ...
  virtual ~PolygonDraw () { }
  virtual int Prep (const pipeline_assembly& pa, const rokz::Device& device);
...
// 
// -------------------------------------------------------------------------
std::shared_ptr<DrawSequence> darkroot::CreatePolygonTextured (const PolygonData& d) {
...
//
// ------------------------------------------------------------------------------------------------
int PolygonDraw::Prep (const pipeline_assembly& pa, const rokz::Device& device) {

  // update_dark_uniform (glob, current_frame,  g.dt); 

  //  fn PolygonDraw::Prep (globals& g, const pipeline_assembly& pa, const rokz::Device& device); 
  // float sim_timef = g.sim_time;
